<div class="dropdown float-md-lift">
    ...
    <div class="dropdown-menu" aria-labelledby="dropdownBreadcrumbButton">
        ...
        <a style="padding: 3px" class="dropdown-item change_status" data-id="{{ $user->id }}"
            data-url = "{{ route('dashboard.users.changeStatus', $user->id) }}" data-lang="{{ app()->getLocale() }}"
            onclick="changeStatus(event,this.dataset.id, this.dataset.url,this.dataset.lang)" href="javascript:void(0)">
            <i class="la @if ($user->status == '1') la-stop @else la-play @endif"></i>
